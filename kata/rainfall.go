...
func Variance(town string, strng string) float64 {
	...
	for _, val := range values {
		res = res + math.Pow((val-mean), 2.0)
	}

	return res / float64(len(values))
}

func extractValues(town string, strng string) []float64 {
	exp := fmt.Sprintf("%s:(.+\\d)", town)
	reg := regexp.MustCompile(exp)
	line := reg.FindString(strng)
	res := []float64{}
	if line != "" {
		numExp := regexp.MustCompile("(\\d+\\.\\d+)")
		values := numExp.FindAllString(line, 100)
		for _, val := range values {
